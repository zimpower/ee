...
EXIT = False
mqtt_id = f'ee_status@{socket.gethostname()}'
...
def main():
    # Parse args:
    parser = argparse.ArgumentParser(description='MQTT interface to Z-Wave sensors.')
    parser.add_argument('mqtt_host', default='localhost', type=str, help='MQTT host')
    parser.add_argument('-U', default=None, type=str, dest='mqtt_user', help='MQTT username')
    parser.add_argument('-p', default=None, type=str, dest='mqtt_pass', help='MQTT password')
    parser.add_argument('--basetopic', default='home', type=str, help='Base topic to publish/subscribe to')
    parser.add_argument('--interface', default='eth0', type=str,
                        dest='interface', help='Interface to use to contact ee')
    parser.add_argument('--time', default=TIME, type=int, dest='time', help='Time between queries')
    ...
    def on_message(client, userdata, msg):
        ...
        try:
            logger.info(f'MQTT Received message: {msg}')
        except Exception as e:
            logger.error(f'MQTT Failed to connect: {str(e)}')
    ...
    while not EXIT:
        ...
        if stats is not None:
            logger.info(f'MQTT Publish: {stats}')
            mqtt_client.publish(mqtt_topic_pub, json.dumps(stats), retain=True)

        # signal.pause()
        logger.debug(f'Sleep for {args.time}mins')
